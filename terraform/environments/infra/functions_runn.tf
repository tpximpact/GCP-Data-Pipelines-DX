...
data "archive_file" "runn_assignments_split_by_day" {
  type        = "zip"
  source_dir  = "../../../cloud_functions/runn/split_by_day"
  output_path = "/tmp/runn_assignments_split_by_day.zip"
}
...
resource "google_cloudfunctions_function" "runn_assignments_split_by_day" {
  name                = "runn_assignments_split_by_day_pipe"
  runtime             = "python312" # of course changeable
  available_memory_mb = 512
  timeout             = 540
  # Get the source code of the cloud function as a Zip compression
  ...
  environment_variables = {
    "DATASET_ID"             = google_bigquery_dataset.runn_raw.dataset_id
    "DESTINATION_TABLE_NAME" = google_bigquery_table.run_assignments_split_by_day.table_id
    "SOURCE_TABLE_NAME"      = google_bigquery_table.runn_assignments.table_id
    "TABLE_LOCATION"         = google_bigquery_dataset.runn_raw.location
    "GOOGLE_CLOUD_PROJECT"   = var.project
  }
}
...
# --------------------------public holidays split by day--------------------------------\
# Generates an archive of the source code compressed as a .zip file.
data "archive_file" "public_holidays_split_by_day" {
  type        = "zip"
  source_dir  = "../../../cloud_functions/runn/split_by_day"
  output_path = "/tmp/public_holidays_split_by_day.zip"
}

# Add source code zip to the Cloud Function's bucket
resource "google_storage_bucket_object" "public_holidays_split_by_day" {
  source       = data.archive_file.public_holidays_split_by_day.output_path
  content_type = "application/zip"

  # Append to the MD5 checksum of the files's content
  # to force the zip to be updated as soon as a change occurs
  name   = "cloud_function-${data.archive_file.public_holidays_split_by_day.output_md5}.zip"
  bucket = data.google_storage_bucket.function_bucket.name
}

resource "google_cloudfunctions_function" "public_holidays_split_by_day" {
  name                = "runn_public_holidays_split_by_day_pipe"
  runtime             = "python312" # of course changeable
  available_memory_mb = 512
  timeout             = 540
  # Get the source code of the cloud function as a Zip compression
  source_archive_bucket = data.google_storage_bucket.function_bucket.name
  source_archive_object = google_storage_bucket_object.public_holidays_split_by_day.name
  ...
  environment_variables = {
    "DATASET_ID"             = google_bigquery_dataset.runn_raw.dataset_id
    "DESTINATION_TABLE_NAME" = google_bigquery_table.runn_public_holidays_split_by_day.table_id
    "SOURCE_TABLE_NAME"      = google_bigquery_table.runn_public_holidays.table_id
    "TABLE_LOCATION"         = google_bigquery_dataset.runn_raw.location
    "GOOGLE_CLOUD_PROJECT"   = var.project
  }
}
...
resource "google_cloudfunctions_function" "rate_cards" {
  ...
  environment_variables = {
    "DATASET_ID"           = google_bigquery_dataset.runn_raw.dataset_id
    "RATES_TABLE_NAME"     = google_bigquery_table.runn_rate_cards.table_id
    "PROJECTS_TABLE_NAME"  = google_bigquery_table.runn_project_rates.table_id
    "TABLE_LOCATION"       = google_bigquery_dataset.runn_raw.location
    "GOOGLE_CLOUD_PROJECT" = var.project
  }
}
