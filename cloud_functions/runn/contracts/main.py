...
def load_config(project_id, service, ingest_time) -> dict:
    return {
        ...
        "table_name": os.environ.get("TABLE_NAME") or "contracts",
...
def process_dataframe(df):
    ...
    df["endDate"] = df["endDate"].apply(
        lambda dateString: (
            pd.Timestamp(f"{dateString}T00:00:00Z") if dateString else None
        )
    )
...
def main(data: dict, context):
    service = "Data Pipeline - Contracts"
    ...
    pages = fetch_all(
        ...
        base_url="https://api.runn.io/contracts/",
        service=service,
        # Max page_size for /contracts is 200
        page_size=200,
    )
