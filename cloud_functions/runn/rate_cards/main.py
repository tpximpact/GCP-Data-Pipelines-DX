...
def load_config(project_id, service) -> dict:
    return {
        ...
        "rates_table_name": (
            os.environ.get("RATES_TABLE_NAME")
            if os.environ.get("RATES_TABLE_NAME")
            else "rate_cards"
        ),
        "projects_table_name": (
            os.environ.get("PROJECTS_TABLE_NAME")
            if os.environ.get("PROJECTS_TABLE_NAME")
            else "rate_project_rates"
        ),
        "location": (
    ...
    for index, row in df.iterrows():
        if row["projectIds"]:
            for project in row["projectIds"]:
                projects.append({"projectId": project, "rateCard": row["id"]})
...
def main(data: dict, context):
    ...
    while True:
        ...
        if response.status_code == 200:
            data = response.json()
            df_rates, df_projects = process_results_page(
                data.get("values", []), df_rates, df_projects
            )
    ...
    df_rates["createdAt"] = df_rates["createdAt"].apply(
        lambda dateString: pd.Timestamp(dateString)
    )
    df_rates["updatedAt"] = df_rates["updatedAt"].apply(
        lambda dateString: pd.Timestamp(dateString)
    )
