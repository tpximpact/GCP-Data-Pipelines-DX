...
def load_config(project_id, service) -> dict:
    return {
        ...
        "table_name": os.environ.get("TABLE_NAME") or "rate_cards",
...
def process_dataframe(df):
    ...
    df["isBlendedRateCard"] = df["isBlendedRateCard"].astype(bool)
    df["blendedRate"] = pd.to_numeric(df["blendedRate"], errors="coerce")
...
def main(data: dict, context):
    ...
    pages = fetch_all(
        ...
        service=service,
        # Max page_size for /rate-cards is 200
        page_size=200,
    )
