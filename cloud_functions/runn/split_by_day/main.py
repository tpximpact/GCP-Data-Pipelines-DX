...
from data_pipeline_tools.util import read_from_bigquery
...
def load_config(project_id, service) -> dict:
    return {
        ...
        "destination_table_name": (
            os.environ.get("DESTINATION_TABLE_NAME")
            if os.environ.get("DESTINATION_TABLE_NAME")
            else "assignments_by_day"
        ),
        "source_table_name": (
            os.environ.get("SOURCE_TABLE_NAME")
            if os.environ.get("SOURCE_TABLE_NAME")
            else "assignments"
        ),
        "location": (
...
def main(data: dict, context):
    ...
    """
        ...
        rows["uniqueId"] = (
            rows["id"].astype(str)
            + "-"
            + rows["startDate"].astype(str)
            + "-"
            + rows["updatedAt"].astype(str)
        )
