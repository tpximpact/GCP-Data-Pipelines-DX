...
def process_actuals(config, process_config):
    ...
    print(
        f"Total actuals fetched from `{project_id}.{dataset_id}.{table_name}`: {len(df)} rows."
    )
...
def update_progress(
    ...
    start_process_time_sql = (
        "NULL"
        if start_process_time is None
        else f"'{start_process_time.strftime('%Y-%m-%d %H:%M:%S')}'"
    )
    last_processed_cursor_sql = (
        "NULL" if last_processed_cursor is None else f"'{last_processed_cursor}'"
    )
    last_successful_write_sql = (
        "NULL" if last_successful_write is None else f"'{last_successful_write}'"
    )
...
def get_last_processed_cursor(progress_table, id):
    ...
    for row in result:
        return (
            row.last_processed_cursor,
            row.last_successful_write,
            row.start_process_time,
        )
...
def main(data: dict, context):
    ...
    next_cursor, last_successful_write, start_process_time = get_last_processed_cursor(
        progress_table, progress_state_id
    )
    ...
    modifiedAfter_dt = (
        last_successful_write.strftime("%Y-%m-%dT%H:%M:%SZ")
