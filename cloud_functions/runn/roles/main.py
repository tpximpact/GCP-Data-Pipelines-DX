...
def load_config(project_id, service, nextCursor) -> dict:
    return {
        "url": "https://api.runn.io/roles/?cursor="+nextCursor,
        "headers": runn_headers(project_id, service),
        "dataset_id": "Runn_Raw",  #os.environ.get("DATASET_ID"),
        "gcp_project": project_id,
        "table_name": "roles", #os.environ.get("TABLE_NAME"),
...
def main(data: dict, context):
    ...
    print(f"Total number of roles fetched: {len(roles)}")

    roles_df = pd.DataFrame(roles)
    write_to_bigquery(config, roles_df, "WRITE_TRUNCATE")
    print("Done")
